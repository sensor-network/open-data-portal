...
const Map = () => {
  ...
  //locations.wait()
  //alert(locations)
  // för säkerhets skull använd optional chaining om datan inte kommit än
  locations?.forEach(l => {
    l.position = [l.position?.lat, l.position?.long]
  });
  ...
  return (
    <MapContainer center={[56.178516, 15.602610]} zoom={13} style={{ height: '100vh', width: '100wh' }}>
      ...
      {locations && locations.map(l => (
              <Marker position={l.position} icon={greenIcon}>
                <Popup>
                  {l.name} <br />
